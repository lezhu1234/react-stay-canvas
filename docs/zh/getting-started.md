...
页面外层可以控制留白、滚动和响应式布局，也可以用沿坐标轴的 CSS 缩放改变显示尺寸。原生指针坐标会在 Event 条件、target 命中和 Listener 运行前换算回 `width` × `height` 场景坐标。显示缩放不会改变场景几何或位图分辨率；需要改变逻辑绘制分辨率时，仍应修改 `width`、`height` 并重建。旋转、倾斜和镜像不属于支持的坐标变换。
