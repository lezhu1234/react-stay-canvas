...
export function ExamplePage({
  ...
  const isCanvasOnly = definition.presentation === "canvas-only"
  const isImmersive = definition.presentation === "immersive" || isCanvasOnly
  const pageClassName = isImmersive
    ? `example-page workspace-page immersive-page${isCanvasOnly ? " canvas-only-page" : ""}`
  ...
  return (
    <article className={pageClassName}>
      ...
      <section
        aria-label={isCanvasOnly ? localized(definition.title) : undefined}
        aria-labelledby={isCanvasOnly ? undefined : "result-tab"}
        className="tab-panel live-panel"
        hidden={!isCanvasOnly && tab !== "result"}
        id="result-panel"
        role={isCanvasOnly ? "region" : "tabpanel"}
      >
        <Demo />
      </section>
