...
import { Button, CanvasCard, colors, DemoLayout, EventLog, placeSceneChild, ResetButton, sceneLine, scenePoint, StatusGrid, TimelineControls, Toolbar } from "../../components/DemoKit"
...
export default function MotionStudioExample() {
  ...
  const mounted = (tools: StayTools) => {
    ...
    panel.appendKeyFrame("panel", new Rectangle({ ...scenePoint(tools, 54, 112), width: 130, height: 112, layer: 1, fillConfig: { color: colors.blueSoft }, strokeConfig: { color: colors.blue, lineWidth: 3 }, transition: { durationMs: 500, type: "easeOutCubic" } }))
    panel.appendKeyFrame("panel", new Rectangle({ ...scenePoint(tools, 286, 68), width: 150, height: 180, layer: 1, fillConfig: { color: colors.greenSoft }, strokeConfig: { color: colors.green, lineWidth: 3 }, transition: { durationMs: 800, delayMs: 120, type: "easeInOutBack" } }))
    panel.appendKeyFrame("panel", new Rectangle({ ...scenePoint(tools, 534, 126), width: 120, height: 92, layer: 1, fillConfig: { color: colors.orangeSoft }, strokeConfig: { color: colors.orange, lineWidth: 3 }, transition: { durationMs: 780, type: "easeOutBounce" } }))

    const connector = tools.createChild({ className: "motion-line" })
    connector.appendKeyFrame("line", animatedLine({ ...sceneLine(tools, 56, 300, 150, 300), layer: 2, strokeConfig: { color: colors.blue, lineWidth: 4, lineCap: "round" }, transition: { durationMs: 500 } }))
    connector.appendKeyFrame("line", animatedLine({ ...sceneLine(tools, 56, 300, 410, 300), layer: 2, strokeConfig: { color: colors.green, lineWidth: 4, lineCap: "round" }, transition: { durationMs: 800, delayMs: 120 } }))
    connector.appendKeyFrame("line", animatedLine({ ...sceneLine(tools, 56, 300, 660, 300), layer: 2, strokeConfig: { color: colors.orange, lineWidth: 4, lineCap: "round" }, transition: { durationMs: 780 } }))

    const caption = tools.createChild({ className: "motion-caption" })
    caption.appendKeyFrame("text", new StayText({ ...scenePoint(tools, 116, 40), text: text("Start", "开始"), font: { size: 20, fontWeight: 700 }, layer: 2, fillConfig: { color: colors.blue }, transition: { durationMs: 500 } }))
    caption.appendKeyFrame("text", new StayText({ ...scenePoint(tools, 360, 40), text: text("Compose", "编排"), font: { size: 28, fontWeight: 700 }, layer: 2, fillConfig: { color: colors.green }, transition: { durationMs: 800, delayMs: 120 } }))
    caption.appendKeyFrame("text", new StayText({ ...scenePoint(tools, 594, 40), text: text("Deliver", "交付"), font: { size: 20, fontWeight: 700 }, layer: 2, fillConfig: { color: colors.orange }, transition: { durationMs: 780 } }))
  ...
  const addGuide = () => {
    ...
    const guide = placeSceneChild(tools, tools.appendChild({
      className: "guide",
