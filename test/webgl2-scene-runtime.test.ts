...
describe("internal WebGL2 scene runtime", () => {
  ...
  it("renders one Standard receiver from a clipped reflected camera without recursion", () => {
    const canvas = document.createElement("canvas")
    canvas.width = 400
    canvas.height = 240
    const gl = createRecordingWebGL2Context(canvas, {
      samples: 4,
      supportedSamples: [4, 2],
    })
    const runtime = new WebGL2SceneRuntime(gl.context)
    const receiver = new Mesh({
      geometry: litTriangle(),
      material: new StandardMaterial({ roughness: 0.24 }),
      planarReflection: {
        localPlane: { point: [0, 0, 0], normal: [0, 0, 1] },
        resolutionScale: 0.5,
      },
    })
    const opaque = new Mesh({
      geometry: triangle(-0.5),
      material: unlit([1, 0.2, 0.1, 1]),
    })
    const glass = new Mesh({
      geometry: litTriangle(-0.25),
      material: new GlassMaterial({ color: [0.5, 0.8, 1, 0.2] }),
    })

    runtime.render([receiver, opaque, glass], camera())

    expect(gl.spies.createFramebuffer).toHaveBeenCalledTimes(4)
    expect(gl.spies.createRenderbuffer).toHaveBeenCalledTimes(4)
    expect(gl.spies.renderbufferStorageMultisample).toHaveBeenCalledTimes(2)
    expect(gl.spies.renderbufferStorage).toHaveBeenCalledTimes(2)
    ...
    expect(gl.spies.shaderSource.mock.calls.some(([, source]) =>
      String(source).includes("u_planar_reflection_view_projection")
      && String(source).includes("apply_clip_plane()")
    ...
    canvas.height = 300
    runtime.render([receiver, opaque, glass], camera())
    expect(gl.spies.createFramebuffer).toHaveBeenCalledTimes(8)
    expect(gl.spies.createRenderbuffer).toHaveBeenCalledTimes(8)
    ...
    runtime.render([receiver, opaque, glass], camera())
    expect(gl.spies.createFramebuffer).toHaveBeenCalledTimes(8)
    expect(gl.spies.deleteFramebuffer).toHaveBeenCalledTimes(6)
    expect(gl.spies.deleteRenderbuffer).toHaveBeenCalledTimes(6)
    runtime.dispose()
    expect(gl.spies.deleteFramebuffer).toHaveBeenCalledTimes(8)
    expect(gl.spies.deleteRenderbuffer).toHaveBeenCalledTimes(8)
  })

  it("rejects multiple planar receivers before mutating GPU resources", () => {
    const canvas = document.createElement("canvas")
    const gl = createRecordingWebGL2Context(canvas)
    const runtime = new WebGL2SceneRuntime(gl.context)
    const receiver = () => new Mesh({
    ...
    expect(() => runtime.render([receiver(), receiver()], camera()))
      .toThrow("at most one planar reflection receiver")
    expect(gl.spies.createProgram).not.toHaveBeenCalled()
    expect(gl.spies.createBuffer).not.toHaveBeenCalled()
    expect(gl.spies.createRenderbuffer).not.toHaveBeenCalled()
    runtime.dispose()
  })
